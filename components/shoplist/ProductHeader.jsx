...
export default function ProductHeader({ productId }) {
  const { cartProducts, currentProduct } = useContextElement();
  const router = useRouter();
  const [productName, setProductName] = useState("Бүтээгдэхүүн");
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (!productId) {
      setProductName("Бүтээгдэхүүн");
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="product-header d-lg-none" style={{
      position: 'sticky',
      top: 0,
      zIndex: 999,
