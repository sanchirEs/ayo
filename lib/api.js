...
export const api = {
  ...
  products: {
    ...
    getById: (id, params = {}) => {
      ...
      return api.fetch(`/products/${id}${query}`, { auth: false });
    },
    getByCategory: (categoryId, params = {}) => {
      const searchParams = new URLSearchParams({ ...params, categoryId });
      return api.fetch(`/products?${searchParams.toString()}`, { auth: false });
    },
    getDetail: (id, params = {}) => {
      const searchParams = new URLSearchParams({ ...params, fields: 'detailed' });
      return api.fetch(`/products/${id}?${searchParams.toString()}`, { auth: false });
    },
    new: (params = {}) => {
      const sp = new URLSearchParams(params);
  ...
  // -------------------------
  orders: {
    create: (orderData) => api.fetch('/orders/createorder-with-payment', {
      method: 'POST',
      body: JSON.stringify(orderData),
      auth: true,
    }),
    createSimple: (orderData) => api.fetch('/orders/createorder', {
      method: 'POST',
      body: JSON.stringify(orderData),
      auth: true,
    }),
    ...
    cancelOrder: (orderId) => api.fetch(`/orders/cancelorder/${orderId}`, { method: 'PUT', auth: true }),
